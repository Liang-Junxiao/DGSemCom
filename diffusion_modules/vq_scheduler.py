...
class VQDiffusionScheduler(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        super(VQDiffusionScheduler, self).__init__()
        ...
        at = torch.tensor(at.astype("float64"), device= config.device)
        bt = torch.tensor(bt.astype("float64"), device= config.device)
        ct = torch.tensor(ct.astype("float64"), device= config.device)
        log_at = torch.log(at)
        log_bt = torch.log(bt)
        log_ct = torch.log(ct)

        att = torch.tensor(att.astype("float64"),device= config.device)
        btt = torch.tensor(btt.astype("float64"),device= config.device)
        ctt = torch.tensor(ctt.astype("float64"),device= config.device)
        ...
        self.adaptive_auxiliary_loss = config.adaptive_auxiliary_loss
        self.auxiliary_loss_weight =  config.auxiliary_loss_weight

        self.num_timesteps = num_train_timesteps

    def forward(self, log_p_x_0_predict, x_0, x_t, timestep):
        t = timestep
        B, L, N, K = log_p_x_0_predict.shape
        x_0 = x_0.reshape(B, L * N)
        x_t = x_t.reshape(B, L * N)

        log_p_x_0_predict= log_p_x_0_predict.reshape(B, L*N, K)
        log_p_x_0_predict = log_p_x_0_predict.permute(0, 2, 1)
        log_onehot_x_0 = index_to_log_onehot(x_0, num_classes= self.num_embed - 1 )

        # log_p_x_t_min_1 = self.q_posterior_star(log_p_x_0_predict, x_t, timestep)
        # log_p_x_t_min_1_true = self.q_posterior_star(log_onehot_x_0, x_t, timestep)
        log_p_x_t_min_1 = self.q_posterior(log_p_x_0_predict, x_t, timestep)
        log_p_x_t_min_1_true = self.q_posterior(log_onehot_x_0, x_t, timestep)
        kl = self.multinomial_kl(log_p_x_t_min_1_true, log_p_x_t_min_1 )
        mask_region = (x_t == self.mask_class).float()
        mask_weight = mask_region * 1. + (1. - mask_region) * 1.
        kl = kl * mask_weight
        kl = sum_except_batch(kl)

        decoder_nll = -log_categorical(log_onehot_x_0, log_p_x_0_predict)
        ...
        if self.auxiliary_loss_weight != 0:
            kl_aux = self.multinomial_kl(log_onehot_x_0, log_p_x_0_predict)
            ...
            aux_loss = addition_loss_weight * self.auxiliary_loss_weight * kl_aux_loss
    ...
    def step(
        ...
        """
        B, L, N, K = model_output.shape
        log_p_x_0_predict= model_output.reshape(B, L*N, K)
        log_p_x_0_predict = log_p_x_0_predict.permute(0, 2, 1)
        sample = sample.reshape(B, L*N)
        if timestep == 0:
        ...
        return x_t_min_1.reshape(B, L, N)
    ...
    def q_posterior(self, log_p_x_0, x_t, t):
        ...
        # log_q_t_given_x_t_min_1[:,-1,:] = self.log_cumprod_ct[t]
        ...
        q = log_p_x_0 - log_q_x_t_given_x_0    # 第一项和第二项实际上都是value，q是求和的加权系数
        # q = log_p_x_0
        log_one_vector = torch.zeros(B, 1, 1).type_as(log_onehot_x_t)
        log_zero_vector = torch.log(log_one_vector + 1.0e-30).expand(-1, -1, L)
        ...
        log_p_x_t_min_1 = q + log_q_t_given_x_t_min_1
        # log_p_x_t_min_1 = q
        # log_p_x_t_min_1 [:,-1,:] = self.log_cumprod_ct[t] - self.log_1_min_cumprod_ct[t]
        log_p_x_t_min_1  = log_p_x_t_min_1  - torch.logsumexp(log_p_x_t_min_1 , dim=1, keepdim=True)
    ...
    def log_Q_t_transitioning_to_known_class(
        ...
        N, L = log_onehot_x_t.shape[1], log_onehot_x_t.shape[2]
        ...
        log_Q_t.masked_scatter_(mask_class_mask, c.expand(-1, N, L))
        if not cumulative:
    ...
    def log_Q_t_transitioning_from_known_class(
        self,  t: torch.int, x_t: torch.LongTensor,  cumulative: bool
    ):
        num_t = t.shape[0]
        t = t.reshape(num_t, 1, 1, 1)
        ...
        one_hot = F.one_hot(x_t, num_classes= self.num_embed)
        Q_t = one_hot * torch.exp(a) + torch.exp(b)
        Q_t[:,:,:, -1] = torch.exp(c).squeeze(3).expand(num_t, 256, 1)


        mask_class_mask = x_t == self.mask_class
        tensor = torch.zeros(self.num_embed, dtype=torch.float32, device= self.device)
        tensor[-1] = 1.
        Q_t[mask_class_mask] = tensor
    ...
    def apply_cumulative_transitions(self, q, t):
        ...
        b = self.log_cumprod_bt[t]
        c = self.log_cumprod_ct[t]
        log_1_min_cumprod_ct = self.log_1_min_cumprod_ct[t]
    ...
    def sample_with_t(self, y, time_step):

        log_Q_t = self.log_Q_t_transitioning_from_known_class(time_step, y,  cumulative= True)

        log_p_y_t = gumbel_noised(log_Q_t, None)

        y_noised = log_p_y_t.argmax(dim= -1)
