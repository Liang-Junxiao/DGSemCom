...
class Distortion(torch.nn.Module):
    def __init__(self, config):
        ...
        self.distortion_metric=config.distortion_metric
        if self.distortion_metric == 'MSE':
            self.dist = MSE()
        elif self.distortion_metric == 'SSIM':
            self.dist = SSIM()
        elif self.distortion_metric == 'MS-SSIM':
            self.dist = MS_SSIM(data_range=1., levels=4, channel=3).cuda()
        elif self.distortion_metric=='LPIPS':
            if lpips is None:
                raise ImportError("LPIPS distortion requires `pip install lpips`.")
            self.dist=lpips.LPIPS(net='alex').cuda()
        else:
            config.logger.info("Unknown distortion type!")
            raise ValueError

    def forward(self, X, Y):
        if self.distortion_metric=='LPIPS':
            return self.dist.forward(X, Y,normalize=True).mean()  # / 255.
        else:
            return  self.dist.forward(X, Y).mean()  # / 255.


if __name__ == '__main__':
    rand_im1 = (torch.randint(0, 255, [1, 3, 256, 128], dtype=torch.float32) / 255.).cuda()
    rand_im2 = (torch.randint(0, 255, [1, 3, 256, 128], dtype=torch.float32) / 255.).cuda()
    losser = MS_SSIM(data_range=1., levels=4, channel=3).cuda()
    losser=lpips.LPIPS(net='alex').cuda()
